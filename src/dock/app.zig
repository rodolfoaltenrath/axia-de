...
pub const App = struct {
    ...
    fn redrawIfNeeded(self: *App) !void {
        ...
        render.drawDock(buffer.cr, self.surface.width, self.surface.height, self.hovered_index);
    ...
    fn updateHover(self: *App) void {
        ...
        const new_hovered = render.hitTest(self.surface.width, self.surface.height, self.pointer_x, self.pointer_y);
    ...
    fn handlePointerButton(data: ?*anyopaque, _: ?*c.struct_wl_pointer, _: u32, _: u32, button: u32, state: u32) callconv(.c) void {
        ...
        app.spawnCommand(catalog.entries[index].command) catch |err| {
