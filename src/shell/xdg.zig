...
pub const XdgManager = struct {
    ...
    pub fn isAppFocusedById(self: *const XdgManager, app_id: []const u8) bool {
        if (app_id.len == 0) return false;
        const view = self.focused_view orelse return false;
    ...
    fn snapSpecAt(self: *const XdgManager, lx: f64, ly: f64) ?SnapPreviewSpec {
        ...
        const top_hard_zone = @as(f64, @floatFromInt(area.y)) + 4.0;
        const top_soft_zone = @as(f64, @floatFromInt(area.y)) + 24.0;
