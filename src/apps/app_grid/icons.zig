...
fn iconAliases(entry: runtime_catalog.AppEntry) []const []const u8 {
    if (std.ascii.eqlIgnoreCase(entry.label, "Terminal")) return &.{ "terminal", "Alacritty", "alacritty", "utilities-terminal" };
    if (std.ascii.eqlIgnoreCase(entry.label, "Firefox")) return &.{ "firefox", "web-browser" };
    if (std.ascii.eqlIgnoreCase(entry.label, "Arquivos")) return &.{ "axia-files", "folder", "folder-open" };
    if (std.ascii.eqlIgnoreCase(entry.label, "VS Code")) return &.{ "vscode", "visual-studio-code", "code" };
    if (std.ascii.eqlIgnoreCase(entry.label, "Configurações")) return &.{ "settings-axia", "preferences-system", "preferences-desktop" };
