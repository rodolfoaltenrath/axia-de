...
pub const State = struct {
    ...
    pub fn moveSelectionWithEntries(
        ...
        pinned_entries: []const runtime_catalog.AppEntry,
        limit: usize,
        delta: isize,
    ) void {
        const current_snapshot = self.snapshotWithEntries(entries, pinned_entries, limit);
    ...
    pub fn snapshotWithEntries(
        ...
        if (self.query_len == 0) {
            const count = @min(pinned_entries.len, capped_limit);
            state.showing_pinned = count > 0;

            for (pinned_entries[0..count], 0..) |entry, index| {
                state.entries[index] = .{
