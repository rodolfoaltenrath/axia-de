const std = @import("std");
const c = @import("client_wl").c;
const chrome = @import("client_chrome");
const model = @import("model.zig");

pub const Rect = chrome.Rect;

pub fn visibleResultLimit(height: u32) usize {
    const total_height = @as(i32, @intCast(height));
    const available = total_height - 208;
    if (available <= 0) return 1;
    return @max(1, @as(usize, @intCast(@divTrunc(available, 62))));
}

pub fn cardRect(width: u32, height: u32, snapshot: model.Snapshot) Rect {
    const total_width = @as(f64, @floatFromInt(width));
    const total_height = @as(f64, @floatFromInt(height));
    const card_width = @min(total_width - 60.0, 680.0);
    const top_margin = 24.0;
    const bottom_margin = 24.0;
    const visible_results = @min(snapshot.count, visibleResultLimit(height));
    const desired_height = if (snapshot.showing_pinned)
        144.0 + @as(f64, @floatFromInt(visible_results)) * 62.0 + 24.0
    else if (snapshot.query.len == 0)
        104.0
    else if (visible_results == 0)
        186.0
    else
        112.0 + @as(f64, @floatFromInt(visible_results)) * 62.0 + 24.0;
    const min_height: f64 = if (snapshot.showing_pinned or snapshot.query.len > 0) 222.0 else 104.0;
    const card_height = @min(total_height - top_margin - bottom_margin, @max(min_height, desired_height));
    return .{
        .x = (total_width - card_width) / 2.0,
        .y = top_margin,
        .width = card_width,
        .height = card_height,
    };
}

pub fn searchRect(width: u32, height: u32, snapshot: model.Snapshot) Rect {
    const card = cardRect(width, height, snapshot);
    return .{
        .x = card.x + 28,
        .y = card.y + 24,
        .width = card.width - 56,
        .height = 46,
    };
}

pub fn resultRect(width: u32, height: u32, snapshot: model.Snapshot, index: usize) Rect {
    const card = cardRect(width, height, snapshot);
    const top_offset: f64 = if (snapshot.showing_pinned) 106.0 else 62.0;
    return .{
        .x = card.x + 28,
        .y = searchRect(width, height, snapshot).y + top_offset + @as(f64, @floatFromInt(index)) * 62.0,
        .width = card.width - 56,
        .height = 56,
    };
}

pub fn hitTest(width: u32, height: u32, x: f64, y: f64, snapshot: model.Snapshot) ?usize {
    for (0..snapshot.count) |index| {
        if (resultRect(width, height, snapshot, index).contains(x, y)) return index;
    }
    return null;
}

pub fn favoriteHitTest(width: u32, height: u32, x: f64, y: f64, snapshot: model.Snapshot) ?usize {
    for (0..snapshot.count) |index| {
        if (favoriteButtonRect(resultRect(width, height, snapshot, index)).contains(x, y)) return index;
    }
    return null;
}

pub fn draw(cr: *c.cairo_t, width: u32, height: u32, snapshot: model.Snapshot, hovered: ?usize) void {
    c.cairo_save(cr);
    defer c.cairo_restore(cr);

    c.cairo_set_operator(cr, c.CAIRO_OPERATOR_SOURCE);
    c.cairo_set_source_rgba(cr, 0, 0, 0, 0);
    c.cairo_paint(cr);
    c.cairo_set_operator(cr, c.CAIRO_OPERATOR_OVER);

    const card = cardRect(width, height, snapshot);

    drawLauncherShadow(cr, card);
    drawRoundedRect(cr, card, 22);
    c.cairo_set_source_rgba(cr, 0.100, 0.100, 0.108, 0.78);
    c.cairo_fill_preserve(cr);
    c.cairo_set_source_rgba(cr, 0.03, 0.035, 0.045, 0.78);
    c.cairo_set_line_width(cr, 1.0);
    c.cairo_stroke(cr);
    drawRoundedRect(cr, .{
        .x = card.x + 1.0,
        .y = card.y + 1.0,
        .width = card.width - 2.0,
        .height = card.height - 2.0,
    }, 21);
    c.cairo_set_source_rgba(cr, 0.58, 0.94, 1.0, 0.08);
    c.cairo_set_line_width(cr, 1.0);
    c.cairo_stroke(cr);

    const search = searchRect(width, height, snapshot);
    drawRoundedRect(cr, search, 12);
    c.cairo_set_source_rgba(cr, 0.055, 0.060, 0.066, 0.74);
    c.cairo_fill_preserve(cr);
    c.cairo_set_source_rgba(cr, 0.36, 0.92, 1.0, 0.82);
    c.cairo_set_line_width(cr, 1.5);
    c.cairo_stroke(cr);

    drawSearchGlyph(cr, .{
        .x = search.x + 14,
        .y = search.y + 12,
        .width = 20,
        .height = 20,
    });

    const placeholder = "Digite para procurar aplicativos ou configurações...";
    const query = if (snapshot.query.len > 0) snapshot.query else placeholder;
    const query_color: [3]f64 = if (snapshot.query.len > 0) .{ 0.95, 0.96, 0.98 } else .{ 0.62, 0.64, 0.68 };
    drawLabel(cr, search.x + 48, search.y + 30, 15, query, query_color[0], query_color[1], query_color[2]);

    if (snapshot.showing_pinned) {
        drawLabel(cr, card.x + 34, search.y + 88, 12.5, "Fixados", 0.68, 0.71, 0.78);
    } else if (snapshot.query.len == 0) return;

    if (snapshot.query.len > 0 and snapshot.count == 0) {
        drawLabel(cr, card.x + 34, search.y + 90, 16, "Nenhum resultado encontrado", 0.92, 0.93, 0.95);
        drawLabel(cr, card.x + 34, search.y + 116, 14, "Tente outro nome, aplicativo ou ajuste do sistema.", 0.67, 0.69, 0.73);
        return;
    }

    for (0..snapshot.count) |index| {
        const rect = resultRect(width, height, snapshot, index);
        const selected = snapshot.selected != null and snapshot.selected.? == index;
        const is_hovered = hovered != null and hovered.? == index;
        drawResultRow(cr, rect, snapshot.entries[index], selected, is_hovered);
    }
}

fn drawResultRow(cr: *c.cairo_t, rect: Rect, entry: model.EntryView, selected: bool, hovered: bool) void {
    if (selected or hovered) {
        drawRoundedRect(cr, rect, 10);
        c.cairo_set_source_rgba(cr, if (selected) 0.24 else 1.0, if (selected) 0.64 else 1.0, if (selected) 0.82 else 1.0, if (selected) 0.16 else 0.07);
        c.cairo_fill_preserve(cr);
        c.cairo_set_source_rgba(cr, 0.62, 0.95, 1.0, if (selected) 0.12 else 0.04);
        c.cairo_set_line_width(cr, 1);
        c.cairo_stroke(cr);
    }

    const icon_rect = Rect{
        .x = rect.x + 16,
        .y = rect.y + 10,
        .width = 38,
        .height = 38,
    };
    drawRoundedRect(cr, icon_rect, 11);
    c.cairo_set_source_rgba(cr, entry.accent[0], entry.accent[1], entry.accent[2], if (entry.enabled) 0.26 else 0.14);
    c.cairo_fill_preserve(cr);
    c.cairo_set_source_rgba(cr, 1, 1, 1, 0.06);
    c.cairo_set_line_width(cr, 1);
    c.cairo_stroke(cr);
    drawCenteredLabel(cr, icon_rect, if (entry.monogram.len > 1) 12 else 16, entry.monogram, if (entry.enabled) 0.97 else 0.76, if (entry.enabled) 0.98 else 0.78, if (entry.enabled) 1.0 else 0.82);

    drawLabel(cr, rect.x + 68, rect.y + 25, 15, entry.label, if (entry.enabled) 0.96 else 0.78, if (entry.enabled) 0.97 else 0.80, if (entry.enabled) 0.99 else 0.83);
    drawLabel(cr, rect.x + 68, rect.y + 45, 13, entry.subtitle, if (entry.enabled) 0.74 else 0.58, if (entry.enabled) 0.76 else 0.60, if (entry.enabled) 0.79 else 0.64);
    if (entry.enabled) {
        drawFavoriteButton(cr, favoriteButtonRect(rect), entry.favorite);
    } else {
        drawLabel(cr, rect.x + rect.width - 84, rect.y + 25, 13, "Em breve", 0.46, 0.78, 0.90);
    }
}

fn favoriteButtonRect(rect: Rect) Rect {
    return .{
        .x = rect.x + rect.width - 92,
        .y = rect.y + 14,
        .width = 76,
        .height = 28,
    };
}

fn drawFavoriteButton(cr: *c.cairo_t, rect: Rect, favorite: bool) void {
    drawRoundedRect(cr, rect, 9);
    c.cairo_set_source_rgba(
        cr,
        if (favorite) 0.34 else 1.0,
        if (favorite) 0.86 else 1.0,
        if (favorite) 1.0 else 1.0,
        if (favorite) 0.26 else 0.075,
    );
    c.cairo_fill_preserve(cr);
    c.cairo_set_source_rgba(cr, 1.0, 1.0, 1.0, if (favorite) 0.10 else 0.05);
    c.cairo_set_line_width(cr, 1.0);
    c.cairo_stroke(cr);
    drawCenteredLabel(
        cr,
        rect,
        13,
        if (favorite) "Fixado" else "Fixar",
        if (favorite) 0.82 else 0.90,
        if (favorite) 0.97 else 0.92,
        if (favorite) 1.0 else 0.95,
    );
}

fn drawSearchGlyph(cr: *c.cairo_t, rect: Rect) void {
    const cx = rect.x + 8;
    const cy = rect.y + 8;
    c.cairo_save(cr);
    defer c.cairo_restore(cr);
    c.cairo_set_source_rgba(cr, 0.90, 0.91, 0.94, 0.96);
    c.cairo_set_line_width(cr, 2.0);
    c.cairo_arc(cr, cx, cy, 5.2, 0, std.math.pi * 2.0);
    c.cairo_stroke(cr);
    c.cairo_move_to(cr, cx + 3.7, cy + 3.7);
    c.cairo_line_to(cr, cx + 9.5, cy + 9.5);
    c.cairo_stroke(cr);
}

fn drawLauncherShadow(cr: *c.cairo_t, card: Rect) void {
    drawRoundedRect(cr, .{
        .x = card.x,
        .y = card.y + 5,
        .width = card.width,
        .height = card.height,
    }, 22);
    c.cairo_set_source_rgba(cr, 0, 0, 0, 0.22);
    c.cairo_fill(cr);
}

fn drawRoundedRect(cr: *c.cairo_t, rect: Rect, radius: f64) void {
    chrome.drawRoundedRect(cr, rect, radius);
}

fn drawLabel(cr: *c.cairo_t, x: f64, y: f64, size: f64, text: []const u8, r: f64, g: f64, b: f64) void {
    chrome.drawLabel(cr, x, y, size, text, r, g, b, c.CAIRO_FONT_WEIGHT_NORMAL);
}

fn drawCenteredLabel(cr: *c.cairo_t, rect: Rect, size: f64, text: []const u8, r: f64, g: f64, b: f64) void {
    chrome.drawCenteredLabel(cr, rect, size, text, r, g, b);
}
