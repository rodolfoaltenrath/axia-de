...
cc \
  "${ROOT_DIR}/shell-v2/axia-dock/main.c" \
  "${GEN_DIR}/xdg-activation-v1-protocol.c" \
  "${GEN_DIR}/wlr-foreign-toplevel-management-unstable-v1-protocol.c" \
  -I"${GEN_DIR}" \
  -o "${TMP_BIN}" \
  $(pkg-config --cflags --libs gtk4 gtk4-layer-shell-0 gio-2.0 gio-unix-2.0 wayland-client gdk-pixbuf-2.0) \
  -lm
